...
describe("S05-S07 completion, reviews, rewards, finance", () => {
  ...
  it("completes the job with a financial summary", async () => {
    ...
    expect(done.body.financial).toMatchObject({ labor: 100, parts: 50, subtotal: 150, commissionRate: 0.1, platformFee: 15, total: 150, technicianEarning: 135 });
  ...
  it("reports technician earnings", async () => {
    ...
    expect(res.body.netEarnings).toBeGreaterThanOrEqual(135);
  });
});
