...
export function CustomerRequestDetailsScreen({ route, navigation }: Props) {
  ...
  const isDeletable = request.state === "pending" || request.state === "cancelled";
  const confirmDelete = () => Alert.alert("حذف الطلب", "هل تريد حذف هذا الطلب نهائياً؟ لا يمكن التراجع عن هذا الإجراء.", [
    { text: "إلغاء", style: "cancel" },
    { text: "حذف", style: "destructive", onPress: () => void repairRequestRepository.remove(request.id).then(() => navigation.goBack()).catch((error: unknown) => Alert.alert("تعذر حذف الطلب", error instanceof Error ? error.message : "حاول مرة أخرى.")) }
  ]);
  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safe}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {isDeletable ? <Pressable accessibilityRole="button" accessibilityLabel="حذف الطلب" onPress={confirmDelete} style={styles.deleteButton}><Ionicons name="trash" size={17} color="#B91C1C" /><LocalizedText style={styles.deleteButtonText}>حذف الطلب</LocalizedText></Pressable> : null}
    </ScrollView>
  </SafeAreaView>;
}
...
const styles = createAdaptiveStyleSheet({
  ...
  deleteButton: { alignItems: "center", backgroundColor: "#FFF1F2", borderColor: "#FECDD3", borderRadius: 15, borderWidth: 1, flexDirection: "row-reverse", gap: 7, justifyContent: "center", minHeight: 46 }, deleteButtonText: { color: "#B91C1C", fontFamily: typography.fontFamily, fontSize: 12, fontWeight: "800" },
