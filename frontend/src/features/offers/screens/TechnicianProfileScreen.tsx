  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safe}>
    <View style={styles.hero}>
      ...
      <View style={styles.heroTop}>
        ...
        <View style={styles.badges}>{technician.isPro ? <View style={styles.proBadge}><Ionicons name="star" size={11} color="#FCD34D" /><Text style={styles.proText}>عَمِّرها Pro</Text></View> : null}{technician.isVerified ? <View style={styles.verifiedBadge}><Ionicons name="shield-checkmark" size={11} color="#6EE7B7" /><Text style={styles.verifiedText}>موثق بالهوية</Text></View> : null}</View>
      </View>
      <Text style={styles.memberSince}>عضوية منذ 3 سنوات • القدس</Text>
    </View>

    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={[styles.card, styles.bioCard]}>
        <View style={styles.bioTop}>
          <View style={styles.identity}><View style={styles.portraitLift}><TechnicianPortrait technician={technician} size={68} /></View><View style={styles.identityCopy}><Text style={styles.name}>{technician.name}</Text><Text style={styles.specialty}>{technician.specialty}</Text></View></View>
          <View style={[styles.available, !technician.isAvailable && styles.unavailable]}><View style={[styles.availableDot, !technician.isAvailable && styles.unavailableDot]} /><Text style={[styles.availableText, !technician.isAvailable && styles.unavailableText]}>{technician.isAvailable ? "متاح الآن" : "غير متاح"}</Text></View>
        </View>
      ...
      <View style={styles.card}>
        <View style={styles.sectionHeading}><Text style={styles.sectionTitle}>معرض الأعمال الموثقة</Text><View style={styles.approved}><Ionicons name="checkmark-circle" size={12} color="#8C6D14" /><Text style={styles.approvedText}>فحص معتمد</Text></View></View>
        <Text style={styles.sectionSubtitle}>قبل وبعد الصيانة</Text>
        <View style={styles.gallery}>
          <View style={[styles.photoWrap, styles.beforeWrap]}><Image source={beforeImage} style={styles.photo} /><View style={[styles.photoLabel, styles.beforeLabel]}><Text style={styles.photoLabelText}>قبل: تسريب وصدأ</Text></View></View>
          <View style={[styles.photoWrap, styles.afterWrap]}><Image source={afterImage} style={styles.photo} /><View style={[styles.photoLabel, styles.afterLabel]}><Text style={styles.photoLabelText}>بعد: عزل وإصلاح متقن</Text></View></View>
        </View>
        <Text style={styles.galleryCaption}>توثيق صيانة حقيقية تم تنفيذها الأسبوع الماضي في حارة السعدية بالبلدة القديمة.</Text>
      </View>

      <View style={styles.card}>
        <View style={styles.sectionHeading}><Text style={styles.sectionTitle}>آراء أهل القدس</Text><Text style={styles.allReviews}>{reviews.length || 154} تقييم موثق</Text></View>
        <View style={styles.reviewCard}>
          <View style={styles.reviewTop}><View><Text style={styles.reviewer}>خليل السلايمة</Text><Text style={styles.reviewerArea}>واد الجوز</Text></View><Text style={styles.stars}>★★★★★</Text></View>
          <Text style={styles.reviewText}>“{review?.comment ?? "ملتزم بالوقت، محترم جداً وما غلّى بالسعر أبداً. فني ابن بلد يعتمد عليه."}”</Text>
          <View style={styles.reviewVerified}><Ionicons name="checkmark-circle" size={11} color="#047857" /><Text style={styles.reviewVerifiedText}>صيانة موثقة عبر عَمِّرها</Text></View>
        </View>
      </View>
    </ScrollView>
