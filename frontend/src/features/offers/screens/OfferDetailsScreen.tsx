...
import { Button, ErrorState, LoadingState, ScreenContainer } from "../../../shared/components";
...
export function OfferDetailsScreen({ route, navigation }: NativeStackScreenProps<CustomerStackParamList, "CustomerOfferDetails">) {
  ...
  useEffect(() => { let active = true; Promise.all([offerRepository.getOffer(route.params.offerId), new DemoTechnicianRepository().getById(route.params.technicianId ?? "")]).then(([found, tech]) => { if (!found || !tech) throw new Error("missing"); if (active) { setOffer(found); setTechnician(tech); } }).catch(() => { if (active) setError("تعذر العثور على العرض أو الفني."); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [route.params.offerId, route.params.technicianId]);
