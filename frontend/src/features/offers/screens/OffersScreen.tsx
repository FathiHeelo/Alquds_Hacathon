...
export function OffersScreen({ route, navigation }: NativeStackScreenProps<CustomerStackParamList, "CustomerOffersEntry">) {
  const state = useOffers(route.params.requestId); const [request, setRequest] = useState<RepairRequest>(); const [technicians, setTechnicians] = useState<readonly Technician[]>([]); const [techniciansFailed, setTechniciansFailed] = useState(false);
  useEffect(() => { let active = true; void repairRequestRepository.getRequest(route.params.requestId).then((value) => { if (active) setRequest(value); }).catch(() => { if (active) setRequest(undefined); }); return () => { active = false; }; }, [route.params.requestId]);
