...
export function CustomerJobEntryScreen({ route, navigation }: NativeStackScreenProps<CustomerStackParamList, "CustomerJobEntry">) { return <ScreenContainer><SuccessState message="تم قبول العرض بنجاح" /><Text style={aiStyles.text}>تم تجهيز الطلب للانتقال إلى الصيانة.</Text><Text style={aiStyles.text}>رقم المهمة: {route.params.jobId}</Text><Text style={aiStyles.text}>الفني: {route.params.technicianId}</Text><Button onPress={() => navigation.popTo("CustomerTabs")}>متابعة إلى التتبع</Button></ScreenContainer>; }
