...
export function OfferCard({ offer, technician, fairPrice, onDetails, onProfile }: { offer: Offer; technician: Technician; fairPrice?: { min: number; max: number }; onDetails(): void; onProfile(): void }) {
  ...
  return <Card><View style={{ gap: spacing.sm }}><View style={{ flexDirection: "row-reverse", justifyContent: "space-between", gap: spacing.sm }}>
    <View style={{ flex: 1, gap: spacing.xs }}><LocalizedText style={[aiStyles.text, { fontWeight: "700" }]}>{technician.name}</LocalizedText><LocalizedText style={[aiStyles.text, aiStyles.muted]}>{technician.specialty}</LocalizedText>
      <LocalizedText style={aiStyles.text}>★ {technician.rating} · {offer.etaMinutes} دقيقة للوصول</LocalizedText></View><LocalizedText style={[aiStyles.price, { color: colors.text }]}>{offer.price} ₪</LocalizedText></View>
    ...
    <LocalizedText style={[aiStyles.text, aiStyles.muted]}>{offer.estimatedDurationMinutes} دقيقة · {offer.message}</LocalizedText><View style={{ flexDirection: "row-reverse", gap: spacing.sm }}>
