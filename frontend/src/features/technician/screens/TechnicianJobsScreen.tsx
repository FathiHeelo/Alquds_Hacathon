...
export function TechnicianJobsScreen() {
  ...
  const jobs = useMemo(() => technicianJobs.filter((job) => filter === "active" ? job.status !== "completed" : job.status === "completed"), [filter]);
  const total = technicianJobs.filter(({ status }) => status === "completed").reduce((sum, job) => sum + job.price, 0);

  return <SafeAreaView edges={["top"]} style={styles.safe}>
    <View style={styles.header}><View><Text style={styles.title}>أعمالي</Text><Text style={styles.subtitle}>إدارة المهام والأرباح والتوثيق</Text></View><View style={styles.headerIcon}><Ionicons name="briefcase" size={21} color={colors.primaryPressed} /></View></View>
    <View style={styles.metrics}><Metric label="مهمة نشطة" value="1" color="#047857" /><Metric label="مكتملة" value="154" /><Metric label="دخل حديث" value={`${total} ₪`} color="#8C6D14" /></View>
    <View style={styles.tabs}><Pressable onPress={() => setFilter("active")} style={[styles.tab, filter === "active" && styles.activeTab]}><Text style={[styles.tabText, filter === "active" && styles.activeTabText]}>الجارية</Text></Pressable><Pressable onPress={() => setFilter("completed")} style={[styles.tab, filter === "completed" && styles.activeTab]}><Text style={[styles.tabText, filter === "completed" && styles.activeTabText]}>المكتملة</Text></Pressable></View>
    <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
      {jobs.map((job) => <Pressable key={job.id} onPress={() => navigation.navigate("TechnicianJobDetails", { jobId: job.id })} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
        <View style={styles.cardTop}><View style={styles.jobIcon}><Ionicons name={job.status === "completed" ? "checkmark" : "navigate"} size={19} color={job.status === "completed" ? "#047857" : "#8C6D14"} /></View><View style={styles.copy}><Text style={styles.problem}>{job.problem}</Text><Text style={styles.customer}>العميل: {job.customerName} • {job.area}</Text></View><View style={[styles.status, job.status === "completed" && styles.completedStatus]}><Text style={[styles.statusText, job.status === "completed" && styles.completedText]}>{job.statusLabel}</Text></View></View>
        <View style={styles.divider} />
        <View style={styles.bottom}><View><Text style={styles.date}>{job.date}</Text><Text style={styles.price}>{job.price} ₪</Text></View><View style={styles.open}><Text style={styles.openText}>تفاصيل العمل</Text><Ionicons name="chevron-back" size={15} color="#8C6D14" /></View></View>
      </Pressable>)}
      {!jobs.length ? <View style={styles.empty}><Ionicons name="briefcase-outline" size={36} color="#CBD5E1" /><Text style={styles.emptyText}>لا توجد أعمال في هذه القائمة</Text></View> : null}
      <View style={styles.note}><Ionicons name="camera" size={17} color="#8C6D14" /><Text style={styles.noteText}>وثّق صور قبل وبعد كل عمل لرفع ثقة العملاء بملفك المهني.</Text></View>
    </ScrollView>
  </SafeAreaView>;
}
