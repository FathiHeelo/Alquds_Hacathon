...
export function TechnicianChatScreen({ route, navigation }: NativeStackScreenProps<TechnicianStackParamList, "TechnicianChat">) {
  ...
  useEffect(() => {
    void chatRepository.getMessages(route.params.conversationId).then((loaded) => setMessages(loaded.length ? loaded : [{ id: "customer-start", sender: "customer", text: "مرحباً، هذه تفاصيل المشكلة كما أرسلتها في الطلب.", createdAt: "10:05 ص" }]));
  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safe}>
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.safe}>
      <View style={styles.header}><Pressable onPress={() => navigation.goBack()} style={styles.headerButton}><Ionicons name="arrow-forward" size={18} color="#475569" /></Pressable><View style={styles.avatar}><Text style={styles.avatarText}>{route.params.customerName.charAt(0)}</Text></View><View style={styles.headerCopy}><View style={styles.nameRow}><Text style={styles.name}>{route.params.customerName}</Text><Ionicons name="lock-closed" size={12} color="#047857" /></View><Text style={styles.problem}>{route.params.problem}</Text></View><View style={styles.headerButton}><Ionicons name="shield-checkmark" size={18} color="#8C6D14" /></View></View>
      <View style={styles.safety}><Ionicons name="shield-checkmark" size={14} color="#176B51" /><Text style={styles.safetyText}>هوية محمية: لا يظهر رقم الهاتف أو العنوان. الاتفاق والدفع داخل عَمِّرها فقط.</Text></View>
