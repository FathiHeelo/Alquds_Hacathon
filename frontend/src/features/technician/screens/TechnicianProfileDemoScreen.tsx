...
export function TechnicianProfileDemoScreen({ navigation }: Props) {
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safe}>
    <View style={styles.header}><Pressable onPress={() => navigation.goBack()} style={styles.back}><Ionicons name="arrow-forward" size={18} color="#475569" /></Pressable><Text style={styles.headerTitle}>ملفي المهني</Text><Pressable onPress={() => Alert.alert("تعديل الملف", "تم فتح نموذج تعديل البيانات المهنية.")} style={styles.edit}><Ionicons name="create-outline" size={18} color="#8C6D14" /></Pressable></View>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <View style={styles.hero}><View style={styles.glow} /><Image source={require("../../../../assets/technicians/tech-tariq-maqdisi.jpg")} style={styles.avatar} /><View style={styles.heroCopy}><View style={styles.nameRow}><Text style={styles.name}>طارق المقدسي</Text><Ionicons name="checkmark-circle" size={16} color="#6EE7B7" /></View><Text style={styles.role}>فني سباكة وصحية • البلدة القديمة</Text><View style={styles.badges}><View style={styles.proBadge}><Ionicons name="star" size={10} color="#FCD34D" /><Text style={styles.proText}>AMMERHA Pro</Text></View><View style={styles.available}><Text style={styles.availableText}>متاح الآن</Text></View></View></View></View>
      ...
      <View style={styles.card}><View style={styles.cardTitleRow}><Ionicons name="person" size={17} color="#8C6D14" /><Text style={styles.cardTitle}>نبذة عني</Text></View><Text style={styles.bio}>فني سباكة معتمد بخبرة 9 سنوات في بيوت القدس، متخصص في كشف التسريبات وصيانة المطابخ والحمامات بسرعة ونظافة.</Text></View>
      <View style={styles.card}><View style={styles.cardTitleRow}><Ionicons name="construct" size={17} color="#8C6D14" /><Text style={styles.cardTitle}>الخدمات</Text></View><View style={styles.chips}><Chip text="كشف التسريبات" /><Chip text="صيانة المجالي" /><Chip text="مضخات المياه" /><Chip text="تمديدات صحية" /></View></View>
      <View style={styles.card}><View style={styles.cardTitleRow}><Ionicons name="images" size={17} color="#8C6D14" /><Text style={styles.cardTitle}>أعمال قبل وبعد</Text></View><View style={styles.portfolio}><View style={styles.photoWrap}><Image source={require("../../../../assets/portfolio/plumbing-before.jpg")} style={styles.photo} /><Text style={styles.photoLabel}>قبل</Text></View><View style={styles.photoWrap}><Image source={require("../../../../assets/portfolio/plumbing-after.jpg")} style={styles.photo} /><Text style={[styles.photoLabel, styles.afterLabel]}>بعد</Text></View></View></View>
      <Pressable onPress={() => navigation.navigate("TechnicianAccountDetail", { section: "services" })} style={styles.primary}><Ionicons name="create" size={17} color={colors.text} /><Text style={styles.primaryText}>تعديل الملف والخدمات</Text></Pressable>
    </ScrollView>
  </SafeAreaView>;
}

function Stat({ icon, value, label }: { icon: keyof typeof Ionicons.glyphMap; value: string; label: string }) { return <View style={styles.stat}><Ionicons name={icon} size={15} color="#C59B27" /><Text style={styles.statValue}>{value}</Text><Text style={styles.statLabel}>{label}</Text></View>; }
function Chip({ text }: { text: string }) { return <View style={styles.chip}><Ionicons name="checkmark" size={11} color="#047857" /><Text style={styles.chipText}>{text}</Text></View>; }
