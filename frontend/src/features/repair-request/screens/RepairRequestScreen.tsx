  ...
  return <SafeAreaView edges={["top", "bottom"]} style={styles.safe}>
    <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.safe}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCopy}><Text style={styles.headerTitle}>{form.reviewing ? "مراجعة طلب الصيانة" : "طلب صيانة جديد"}</Text><Text style={styles.headerSubtitle}>بصوتك أو بالصورة • تشخيص فوري</Text></View>
        <View style={styles.headerSpacer} />
      </View>

      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        {form.reviewing ? <>
          <View style={styles.infoBanner}><Ionicons name="checkmark-circle" size={19} color="#047857" /><Text style={styles.infoText}>تأكد من التفاصيل قبل إرسالها للفنيين القريبين منك.</Text></View>
          <View style={styles.card}><RequestReview draft={draft} /></View>
          <Pressable disabled={disabled} onPress={form.edit} style={styles.secondaryButton}><Ionicons name="create-outline" size={17} color={colors.text} /><Text style={styles.secondaryButtonText}>تعديل التفاصيل</Text></Pressable>
        </> : <>
          <View style={[styles.card, styles.voiceCard]}>
            <View style={styles.cardHeading}>
              <View style={styles.headingRow}><View style={styles.aiIcon}><Text style={styles.aiText}>AI</Text></View><Text style={styles.cardTitle}>احكِ المشكلة بصوتك</Text></View>
              <View style={styles.readyBadge}><View style={styles.readyDot} /><Text style={styles.readyText}>{form.busy === "voice" ? "جارٍ التحليل" : "جاهز للتسجيل"}</Text></View>
            </View>
            <View style={styles.waveBox}>
              <View style={styles.waveform}>{[16, 28, 38, 22, 34, 18, 30, 12, 25].map((height, index) => <View key={index} style={[styles.wave, { height }]} />)}</View>
              {transcript ? <View style={styles.transcript}><Text style={styles.transcriptLabel}>التفريغ الصوتي التلقائي:</Text><Text style={styles.transcriptText}>“{transcript}”</Text></View> : <Text style={styles.voiceHint}>اضغط التسجيل واحكِ المشكلة بطريقتك، وسنرتب التفاصيل تلقائيًا.</Text>}
            </View>
            {form.suggestion ? <View style={styles.voiceActions}><Pressable onPress={form.dismissVoice} style={styles.smallGhost}><Text style={styles.smallGhostText}>إعادة التسجيل</Text></Pressable><Pressable onPress={form.applyVoice} style={styles.smallPrimary}><Text style={styles.smallPrimaryText}>اعتماد النص</Text></Pressable></View> : <Pressable disabled={disabled} onPress={() => void form.loadVoice()} style={styles.recordButton}><View style={styles.recordDot} /><Text style={styles.recordText}>{form.busy === "voice" ? "جارٍ تجهيز التسجيل…" : "ابدأ التسجيل الصوتي"}</Text></Pressable>}
          </View>

          <View style={styles.section}><Text style={styles.label}>وصف المشكلة</Text><TextInput multiline value={draft.description} onChangeText={(description) => form.update({ description })} placeholder="اشرح العطل باختصار..." placeholderTextColor="#94A3B8" style={styles.textArea} />{errors.description ? <Text style={styles.error}>{errors.description}</Text> : null}</View>

          <View style={styles.section}><Text style={styles.label}>نوع الخدمة</Text><View style={styles.chips}>{serviceCategories.map((category) => { const selected = draft.category === category.id; return <Pressable key={category.id} disabled={disabled} onPress={() => form.update({ category: category.id })} style={[styles.chip, selected && styles.selectedChip]}><Text style={[styles.chipText, selected && styles.selectedChipText]}>{category.label}</Text></Pressable>; })}</View>{errors.category ? <Text style={styles.error}>{errors.category}</Text> : null}</View>

          <View style={styles.section}><Text style={styles.label}>أرفق صورة أو فيديو للعطل</Text><Pressable disabled={disabled} onPress={() => void form.attachMedia()} style={styles.mediaBox}><View style={styles.mediaIcon}><Ionicons name="camera" size={23} color={colors.primaryPressed} /></View><Text style={styles.mediaTitle}>{form.busy === "media" ? "جارٍ فتح المعرض…" : draft.media.length ? "إضافة صورة أو فيديو آخر" : "صوّر العطل أو اختر من المعرض"}</Text><Text style={styles.mediaHint}>الصورة تساعد الذكاء الاصطناعي على تشخيص أدق</Text></Pressable><RequestMediaList media={draft.media} onRemove={form.removeMedia} disabled={disabled} /></View>

          <View style={styles.section}><Text style={styles.label}>موقع الصيانة بالقدس</Text><View style={styles.locationBox}><View style={styles.locationIcon}><Ionicons name="location" size={19} color="#8C6D14" /></View><TextInput value={draft.location.label} onChangeText={(label) => form.update({ location: { ...draft.location, label } })} style={styles.locationInput} /><View style={styles.autoBadge}><Text style={styles.autoText}>محدد تلقائياً</Text></View></View>{errors.location ? <Text style={styles.error}>{errors.location}</Text> : null}</View>
      ...
      <View style={styles.footer}><Pressable disabled={disabled} onPress={() => void continueFlow()} style={({ pressed }) => [styles.submitButton, disabled && styles.disabled, pressed && styles.pressed]}><Ionicons name={form.reviewing ? "checkmark-circle" : "sparkles"} size={18} color={colors.text} /><Text style={styles.submitText}>{form.busy === "submit" ? "جارٍ إرسال الطلب…" : form.reviewing ? "تأكيد الطلب والمتابعة" : "تشخيص العطل واقتراح السعر العادل"}</Text></Pressable></View>
